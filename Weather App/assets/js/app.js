    ...
    searchTimeout = setTimeout(() => {
      fetchData(url.geo(searchField.value), function (locations) {
        ...
        if (locations.length === 0) {
          ...
          searchItem.innerHTML = `<p class="search-not-found">
                                    <span class="nothing">Nothing Found!</span> 
                                    Please try with different city or country
                                  </p>`;
          searchResult
...
export const updateWeather = function (lat, lon) {
  ...
  fetchData(url.currentWeather(lat, lon), function (currentWeather) {
    ...
    card.innerHTML = `
        <h2 class="title-2 card-title" style="display: flex; gap:.5rem">Now:
            <span class="meta-text" data-location-s></span>
        </h2>
        <div class="weapper">
    ...
    fetchData(url.reverseGeo(lat, lon), function ([{ name, country }]) {
      card.querySelector('[data-location-s]').innerHTML = `${name}, ${country}`;
      ...
      card.innerHTML = `
        ...
        module.aqiText[aqi].message
            ...
            <div class="card card-sm highlight-card">
                ...
                <div class="wrapper">
                    <span class="m-icon">airwaves</span>
    ...
    fetchData(url.forecast(lat, lon), function (forecast) {
      ...
      hourlySection.innerHTML = `
        <h2 class="title-2">Hourly Forcast</h2>
      ...
      const footerHtml = `
            <footer class="footer">
                <p class="body-3 link-linkedin">
                    Copyright &copy; 2023
                    <a href="https://www.linkedin.com/in/chnoumanejaz/" target="_blank" class="linkedin"> Nouman Ejaz. </a>
                    All Rights Reserved.
                </p>
                <p class="body-3">
                    Powered by
      ...
      hourlySection.insertAdjacentHTML('beforeend', footerHtml);
    });
  });
};

const themeIcon = document.querySelector('.theme-icon');
themeIcon.innerHTML = `<span class="m-icon">clear_day</span>`;
const mainBody = document.querySelector('body');

themeIcon.addEventListener('click', function () {
  mainBody.classList.toggle('light-theme');
  if (themeIcon.innerHTML === '<span class="m-icon">clear_day</span>') {
    themeIcon.innerHTML = `<span class="m-icon">clear_night</span>`;
    notification('Theme Changed to Light Mode :(');
    localStorage.setItem('theme', 'day');
  } else {
    themeIcon.innerHTML = `<span class="m-icon">clear_day</span>`;
    notification('Theme Changed to Dark Mode :)');
...
/**
 * Send Notifications on Message
 */
function notification(msg) {
  if ('Notification' in window) {
    Notification.requestPermission().then(function (permission) {
      if (permission === 'granted') {
        let notification = new Notification('WeatherNow! (Nouman Ejaz)', {
          body: `${msg}`,
        });
      }
    });
